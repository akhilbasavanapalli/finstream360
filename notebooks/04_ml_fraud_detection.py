...
def build_feature_df(df):
    ...
    return (
        df
        .withColumn("cross_state_flag",  F.col("is_cross_state").cast(DoubleType()))
...
balanced_df = nonfraud_df.union(
    fraud_df.sample(withReplacement=True, fraction=float(oversample_factor), seed=42)
)
...
cat_encoders  = [OneHotEncoder(inputCol=f"{c}_idx", outputCol=f"{c}_ohe")
                 for c in CATEGORICAL_FEATURES]
...
feature_names = (
    NUMERIC_FEATURES
    + [f"{c}_ohe_{i}" for c in CATEGORICAL_FEATURES for i in range(5)]  # approx
)

importances = sorted(
    zip(feature_names[:len(gbt_model.featureImportances)],
...
(
    scored_df.write
             .format("delta")
             .mode("overwrite")
             .save(f"{GOLD_PATH}/fraud_predictions")
)
